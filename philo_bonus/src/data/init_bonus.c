...
/*   Updated: 2025/02/03 17:41:47 by danpalac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_semaphores(t_data **data)
{
	...
	if ((*data)->forks == SEM_FAILED)
		return (0);
	(*data)->sem = sem_open("semaphores", O_CREAT, 0644, MAX_SEM);
	if ((*data)->sem == SEM_FAILED)
		return (0);
	(*data)->state = ALIVE_STATE;
	return (1);
}
...
static int	init_data(t_data **data, char **av)
{
	*data = (t_data *)ft_calloc(1, sizeof(t_data));
	if (!*data)
		return (0);
	(*data)->pid = ft_calloc(ft_atoi(av[1]), sizeof(pid_t));
	if (!(*data)->pid)
		return (0);
...
static int	init_philos(t_philo **philos, t_data *data)
{
	...
	*philos = (t_philo *)ft_calloc(data->n_philos, sizeof(t_philo));
	if (!*philos)
		return (0);
	i = 0;
